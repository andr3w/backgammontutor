...
const script = f => `<script${f.endsWith('.mjs') ? ' type="module"' : ''} src="/static/${stamp(f)}"></script>`;
...
function screens(tree) {
  ...
  for (const kid of tree.kids) {
    if (typeof kid !== 'string' && kid.meta) {
      ...
      const opening = R.plays(m.board, side, R.roll(m.dice)).map(p => p[0]);
      ...
      out.push(el('section', { class: 'screen screen-question', id: m.id, 'data-q': payload(m, side) }, [
...
function payload(m, side) {
  return JSON.stringify({
    ...
    goals: (m.goals || []).map(g => ({ kind: g.kind, arg: g.arg, why: g.why ? asHtml(g.why) : null })),
