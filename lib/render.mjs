...
function questionScreen(m, lead) {
  const side = m.turn || 'x';
...
function endScreen(tail, p) {
  const onward = p.next
    ? el('a', { class: 'onward', href: '/' + p.next }, ['Next page'])
  ...
  // which ones are won.
  const medals = p.questions.map((q, i) => {
    const roll = q.dice.join('-');
    const twice = p.questions.filter(o => o.dice.join('-') === roll).length > 1;
    return el('li', { class: 'medal-slot', 'data-for': q.id }, [
      el('a', { href: '#' + q.id }, [
        el('span', { class: 'pip' }, ['\u2605']),
        el('span', { class: 'roll' }, [twice ? `${roll} (${i + 1})` : roll]),
      ]),
    ]);
  });
 ...
 */
function screens(tree, p) {
  const out = [];
  let run = [];
  let first = true;
  for (const kid of tree.kids) {
    if (typeof kid !== 'string' && kid.meta) {
      if (first) { out.push(questionScreen(kid.meta, run.length ? run : null)); first = false; }
      else {
        if (run.length) out.push(el('section', { class: 'screen screen-text' },
          [el('div', { class: 'text-inner' }, run)]));
        out.push(questionScreen(kid.meta, null));
      }
      run = [];
    } else {
      run.push(kid);
  ...
  out.push(endScreen(run, p));
  return out;
}

export function renderPage(p) {
  const body = el('main', { class: 'pager' }, screens(p.tree, p));
