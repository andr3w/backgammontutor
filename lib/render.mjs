...
import { $m, html, inline } from './kit.mjs';
import { GLOSSARY, SOURCES, slugOf } from './glossary.mjs';
...
// --- the glossary ---------------------------------------------------------
//
// Written into every page rather than fetched, so a tapped word opens without
// a round trip and works offline. It is about 12kB, which is less than one
// board's worth of SVG.
function glossaryList() {
  return el('dl', { class: 'gloss-list' }, GLOSSARY.flatMap(e => {
    const src = SOURCES[e.src];
    const see = (e.see || []).map((t, i) => [
      i ? ', ' : '',
      el('a', { class: 'gloss', 'data-term': t.toLowerCase(), href: '/glossary#' + slugOf(t) }, [t]),
    ]).flat();
    return [
      el('dt', { id: slugOf(e.term) }, [e.term]),
      el('dd', {}, [
        ...inline(e.def.replace(/\s+/g, ' ').trim()),
        ...(see.length ? [el('span', { class: 'see' }, ['See also ', ...see, '.'])] : []),
        ...(src ? [el('a', { class: 'src', href: src.url, target: '_blank',
                             rel: 'noopener', title: src.title }, [src.name])] : []),
      ]),
    ];
  }));
}

const glossaryHead = extra => el('div', { class: 'gloss-head' }, [
  el('h2', {}, ['Glossary']),
  ...extra,
]);

function glossaryPane() {
  return el('aside', { class: 'gloss-pane', hidden: '', 'aria-label': 'Glossary' }, [
    glossaryHead([
      el('a', { class: 'gloss-open', href: '/glossary', target: '_blank', rel: 'noopener' }, ['Open in a tab']),
      el('button', { type: 'button', class: 'gloss-close', 'aria-label': 'Close glossary' }, ['\u00d7']),
    ]),
    el('div', { class: 'gloss-body' }, [glossaryList()]),
  ]);
}

export function renderGlossary() {
  const body = el('main', { class: 'gloss-page' }, [
    glossaryHead([el('a', { class: 'gloss-open', href: '/' }, ['All pages'])]),
    el('div', { class: 'gloss-body' }, [glossaryList()]),
  ]);
  return shell('Glossary', html(body), ['gloss.mjs']);
}
...
// escaped.
// A goal written as a plain string gets the same inline markup a prose
// template would -- otherwise **bold** and [[glossary]] links work in one and
// silently do not in the other.
const asHtml = v => (typeof v === 'string' ? html($m('p', inline(v))) : html(v));
...
function payload(m, side) {
  return JSON.stringify({
    ...
    antigoals: (m.antigoals || []).map(g => ({
      kind: g.kind, arg: g.arg, why: g.why ? asHtml(g.why) : null,
    })),
...
export function renderPage(p, titles = {}) {
  ...
  return shell(p.title, html(body) + html(glossaryPane()),
    ['m.js', 'nav.js', 'play.mjs', 'gloss.mjs']);
}
...
export function renderIndex(pages) {
  ...
  return shell('Backgammon Tutor', html(body), ['gloss.mjs']);
}
